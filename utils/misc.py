...
def format_args(args, sorted_key: bool = True) -> str:
    ...
    for _k in key_list:
        ...
        _v = str(args[_k])
        # _cont += f"| {_k:<{m_l}s} =  {_v}\n"
        _cont += f"| {_k.ljust(m_l)} = {_v.ljust(30)} {'|'.rjust(36 - m_l)}\n"
    _cont += '=' * (60 + m_l)  # + '\n'
    return _cont
...
def check_param_loading(not_pretrained_params: list[str], loaded_params: list[str]):
    for n in not_pretrained_params:
        assert n in loaded_params, f"{n}; {loaded_params}"
