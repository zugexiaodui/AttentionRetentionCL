...
from utils.gvm import GlobalVarsManager as GVM
...
@register_model
def vit_base_patch16_224_ibot1k_block(pretrained: bool = False, **kwargs) -> VisionTransformer:
    model_kwargs = dict(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, **kwargs)
    model = _create_vision_transformer('vit_base_patch16_224.augreg_in21k', pretrained=False, **model_kwargs)
    path = 'ibot.pth' # Replace with the actual path to the iBOT checkpoint
    dst_state_ckpt = torch.load(path, map_location='cpu')['state_dict']
    for k in list(dst_state_ckpt.keys()):
        if k.startswith('head.'):
            dst_state_ckpt.pop(k)
    missing_keys, unexpected_keys = model.load_state_dict(dst_state_ckpt, strict=False)
    assert len(unexpected_keys) == 0, f"Unexpected keys found in state_dict: {unexpected_keys}"
    for _k in missing_keys:
        assert 'head' in _k or 'lora' in _k, f"Missing key found in state_dict: {_k}"
    return model
